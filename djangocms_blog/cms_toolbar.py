...
from .models import BLOG_CURRENT_POST_IDENTIFIER, NEWS_CURRENT_POST_IDENTIFIER
...
@toolbar_pool.register
class NewsToolbar(BlogToolbar):

    def populate(self):
        # TODO: Readd if not self.is_current_app condition when CMS 3.0.4 is released
        if not self.request.user.has_perm('djangocms_blog.add_post'):
            return   # pragma: no cover
        admin_menu = self.toolbar.get_or_create_menu('djangocms_blog_news', _('News'))
        url = reverse('admin:djangocms_blog_newspost_changelist')
        admin_menu.add_modal_item(_('Post list'), url=url)
        url = reverse('admin:djangocms_blog_newspost_add')
        admin_menu.add_modal_item(_('Add post'), url=url)

        current_post = getattr(self.request, NEWS_CURRENT_POST_IDENTIFIER, None)
        if current_post and self.request.user.has_perm('djangocms_blog.change_post'):   # pragma: no cover
            admin_menu.add_modal_item(_('Edit Post'), reverse(
                'admin:djangocms_blog_newspost_change', args=(current_post.pk,)),
                active=True)
